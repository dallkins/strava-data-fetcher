...
class EmailNotifier:
    ...
    def __init__(self, config):
        ...
        self.recent_emails = {}  # Track recent emails to prevent duplicates
    ...
    def send_immediate_notification(self, activity_id, event_type):
        """Send immediate notification for new/updated ride to specific athlete - called AFTER DB update"""
        try:
            logging.info(f"Sending {event_type} notification for activity {activity_id}")
            
            # Email deduplication - add this at the start
            email_key = f"{activity_id}_{event_type}"
            current_time = time.time()
            
            # Check if we already sent this email recently
            if hasattr(self, 'recent_emails'):
                if email_key in self.recent_emails:
                    time_diff = current_time - self.recent_emails[email_key]
                    if time_diff < 1800:  # Don't send same email within 30 minutes
                        logging.info(f"Skipping duplicate email for {email_key} (sent {time_diff:.0f}s ago)")
                        return
            else:
                # Initialize recent_emails if it doesn't exist
                self.recent_emails = {}
            ...
            if not athlete_email:
                logging.warning(f"No email configured for athlete {athlete_name}")
                return
            ...
            logging.info(f"DEBUG: Getting 7-day stats from {seven_days_ago} to {today}")
            ...
            # Get week-to-date stats (Monday to today)
            days_since_monday = today.weekday()  # Monday = 0
            this_monday = today - timedelta(days=days_since_monday)
            
            logging.info(f"DEBUG: Getting WTD stats from {this_monday} to {today}")
            wtd_stats = self.get_activity_stats(
                athlete_name=activity['athlete_name'],
            ...
            athlete_week_stats = self._get_athlete_stats_flexible(week_stats, activity['athlete_name'])
            athlete_wtd_stats = self._get_athlete_stats_flexible(wtd_stats, activity['athlete_name'])
            
            # Debug logging
            logging.info(f"DEBUG: 7-day stats for '{activity['athlete_name']}': {athlete_week_stats}")
            logging.info(f"DEBUG: WTD stats for '{activity['athlete_name']}': {athlete_wtd_stats}")
            ...
            <body style="font-family: Arial, sans-serif; color: #333;">
                ...
                <div style="background: #f3e5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
                    ...
                    <p><strong>Rides This Week:</strong> {self.format_number(athlete_wtd_stats['rides'])}</p>
                    <p><strong>Distance This Week:</strong> {self.format_number(athlete_wtd_stats['distance_km'])} km</p>
                    <p><strong>Elevation This Week:</strong> {self.format_number(athlete_wtd_stats['elevation_m'])} m</p>
                    <p><strong>Calories This Week:</strong> {self.format_number(athlete_wtd_stats['calories'])}</p>
                    
                    {self.get_calorie_equivalents(athlete_wtd_stats['calories'])}
                </div>
            ...
            logging.info(f"Sent {event_type} notification to {activity['athlete_name']} ({athlete_email}) for activity {activity_id}")
            
        except Exception as e:
            logging.error(f"Error sending immediate notification: {e}")

    def _get_athlete_stats_flexible(self, stats_dict, athlete_name):
        """Flexible lookup for athlete stats with case-insensitive matching"""
        if not stats_dict:
            return {'rides': 0, 'distance_km': 0, 'elevation_m': 0, 'calories': 0, 'moving_time_hours': 0}
        
        # Try different variations of the athlete name
        athlete_name_variations = [
            athlete_name,  # Exact match
            athlete_name.lower(),  # Lowercase
            athlete_name.capitalize(),  # Capitalized
            athlete_name.title(),  # Title case
        ]
        
        for name_variant in athlete_name_variations:
            if name_variant in stats_dict:
                return stats_dict[name_variant]
        
        # If nothing found, return empty stats
        logging.warning(f"No stats found for athlete '{athlete_name}' in {list(stats_dict.keys())}")
        return {'rides': 0, 'distance_km': 0, 'elevation_m': 0, 'calories': 0, 'moving_time_hours': 0}
...
class ActivityRefreshManager:
    ...
    def fetch_and_save_activity(self, activity_id, athlete_id, athlete_name):
        """Fetch missing activity from Strava and save to database with enhanced logging"""
        try:
            logging.info(f"Starting fetch for activity {activity_id}, athlete: {athlete_name} (ID: {athlete_id})")
            ...
            async def fetch_activity():
                ...
                if athlete_name.lower() == 'dominic':
                    ...
                    logging.info(f"Using Dominic's token: {access_token[:10]}... (expires: {expires_at})")
                elif athlete_name.lower() == 'clare':
                    access_token = os.getenv('CLARE_ACCESS_TOKEN')
                    expires_at = int(os.getenv('CLARE_TOKEN_EXPIRES', '0'))
                    logging.info(f"Using Clare's token: {access_token[:10]}... (expires: {expires_at})")
                
                if not access_token:
                    logging.error(f"No access token found for {athlete_name}")
                    logging.info(f"Available env vars: {[k for k in os.environ.keys() if 'TOKEN' in k]}")
                    return None
                
                # Check if token is expired or will expire soon (refresh 1 hour early)
                current_time = int(time.time())
                logging.info(f"Current time: {current_time}, Token expires: {expires_at}")
                ...
                if current_time >= (expires_at - 3600):
                    ...
                    logging.info(f"New token obtained: {access_token[:10]}...")
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, headers=headers) as response:
                        ...
                        if response.status == 200:
                            ...
                            logging.info(f"Successfully fetched activity data: {activity_data.get('name', 'Unknown')}")
                            ...
                            if success:
                                ...
                                async with session.get(url, headers=headers) as retry_response:
                                    logging.info(f"Retry response status: {retry_response.status}")
                                    if retry_response.status == 200:
                                        activity_data = await retry_response.json()
                                        logging.info(f"Successfully fetched activity on retry: {activity_data.get('name', 'Unknown')}")
                                        return activity_data
                                    else:
                            ...
                            logging.error(f"Rate limited! Response headers: {dict(response.headers)}")
                            return None
                        else:
            ...
            if activity_data:
                logging.info(f"Got activity data, now parsing and saving to database...")
                
                # Parse and save the activity
                activity = parse_strava_activity(activity_data, athlete_name, athlete_id)
                logging.info(f"Parsed activity: {activity.name}")
                ...
                logging.info(f"Executing database insert/update for activity {activity_id}")
                cursor.execute(insert_sql, values)
                
                affected_rows = cursor.rowcount
                logging.info(f"Database operation completed. Affected rows: {affected_rows}")
                ...
                logging.error(f"No activity data received from Strava API for activity {activity_id}")
                return False
...
def clear_email_cache():
    ...
    if hasattr(email_notifier, 'recent_emails'):
        email_notifier.recent_emails = {}
    return jsonify({"status": "success", "message": "Email cache cleared"})

@app.route('/debug-fetch/<int:activity_id>', methods=['POST'])
def debug_fetch_activity(activity_id):
    """Debug endpoint to test activity fetching"""
    try:
        # Get activity details from DB first
        conn = refresh_manager.get_connection()
        cursor = conn.cursor()
        ...
        logging.info(f"DEBUG: Manual fetch requested for activity {activity_id} ({athlete_name})")
        ...
        logging.error(f"Error in debug fetch: {e}")
        return jsonify({"error": str(e)}), 500
...
def webhook_stats():
    ...
    try:
        ...
        result = cursor.fetchone()
        pending_refreshes = result[0] if result else 0
